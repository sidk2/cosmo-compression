...
dataset = data.CelebA64Dataset('../../monolith/global_data/astro_compression/celeba_64', 'val', transform)

img = dataset[1]

mean = torch.mean(img, dim=[-1,-2]).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).cuda()
std = torch.std(img, dim=[-1,-2]).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).cuda()
img = (img.unsqueeze(0).cuda() - mean)/std

fm: lightning.LightningModule = represent.Represent.load_from_checkpoint(
    "time_to_code_faces_w_gdn/step=step=33000-val_loss=0.025.ckpt"
).to('cuda')
...
ax[0].imshow((img[0, :, : , :]*std[0, :, : , :]+mean[0, :, : , :]).detach().cpu().permute(1, 2, 0).numpy())
ax[1].imshow((pred[0, :, : , :]*std[0, :, : , :]+mean[0, :, : , :]).detach().cpu().permute(1, 2, 0).numpy())
ax[0].set_title("x")
