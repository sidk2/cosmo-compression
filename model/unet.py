...
class UNetConv(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, t: torch.Tensor | None = None, z: torch.Tensor | None = None
    ) -> torch.Tensor:
        """Overloads forward method of nn.Module"""
        # t is shape [batch_size]
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.down2 = DownStep(
            in_channels=128 + self.num_latent_channels,
            out_channels=256,
            time_dim=time_dim,
        )
        self.sa2 = SelfAttention(channels=256)
        self.down3 = DownStep(
            in_channels=256 + self.num_latent_channels,
            out_channels=512,
            time_dim=time_dim,
        )
        self.sa3 = SelfAttention(channels=512)
        self.down4 = DownStep(
            in_channels=512 + self.num_latent_channels,
            out_channels=512,
            time_dim=time_dim,
        )
        ...
        self.up1 = UpStep(
            in_channels=256,
            res_channels=256 + self.num_latent_channels,
            out_channels=256,
            time_dim=time_dim,
        )
        ...
        self.up2 = UpStep(
            in_channels=256,
            res_channels=128 + self.num_latent_channels,
            out_channels=128,
            time_dim=time_dim,
        )
        self.up3 = UpStep(
    ...
    def forward(
        ...
        # Upsampling stages
        x = self.up0(
            x5, x4, t, repr[:, 1280: 1536]
        )
        # x = self.sa0_inv(x)
        x = self.up1(
            x, x3, t, repr[:, 1536:1792]
        )
        x = self.sa1_inv(x)
        x = self.up2(
            x, x2, t, repr[:, 1792:2048]
        )
        # x = self.sa2_inv(x)
