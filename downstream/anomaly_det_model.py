...
class AnomalyDetectorImg(nn.Module):
    def __init__(self, hidden, dr, channels):
        ...
        self.C51 = nn.Conv2d(32*hidden, 32*hidden, kernel_size=3, stride=1, padding=1,
                            padding_mode='circular', bias=True)
        ...
        self.C61 = nn.Conv2d(32*hidden, 16*hidden, kernel_size=4, stride=1, padding=0,
                            padding_mode='circular', bias=True)
        self.B61 = nn.BatchNorm2d(16*hidden)

        self.P0  = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)

        self.FC1  = nn.Linear(16*hidden, 64*hidden)  
